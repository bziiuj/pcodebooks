...
int main( int argc, char** argv )
{
    ...
    //PD correction, reading iagrams first, before computing the Gram matrix:
    std::vector< std::vector< double > > births(num_datasets);
    std::vector< std::vector< double > > deaths(num_datasets);
    for( int64_t idx_1 = 0; idx_1 < num_datasets; idx_1++ ) 
    {
		const std::string& input_filenames1 = input_filenames[ idx_1 ];
		std::vector< double > bds;	
		std::vector< double > dts;
		get_selected_points( input_filenames1, dim, bds, dts );
		births[idx_1] = bds;
		deaths[idx_1] = dts;
	}
	...
	//PD correcton: and here we only compute the gram matrix.    
    for( int64_t idx_1 = 0; idx_1 < num_datasets; idx_1++ ) {       
        std::vector< double > births_1 = births[idx_1];
        std::vector< double > deaths_1 = deaths[idx_1];
        

        // loop over upper triangular part (without diagonal)
        for( int64_t idx_2 = idx_1 + 1; idx_2 < num_datasets; idx_2++ ) {            
            std::vector< double > births_2 = births[idx_2];
            std::vector< double > deaths_2 = deaths[idx_2];               
            double value = 0.0;
            if( inner_product )
#ifdef USE_FGT
                if ( use_fgt )
