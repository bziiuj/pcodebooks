function [ToyData]=generate_shape_data_new(n,m,V)
%generates 6 classes of toy data to use in classification experiments
...
%Column 6: torus
%Column 7: Random Point cloud (in R^2)

%Random points drawn from a uniform distribution
ToyData{1,1}=['Random Cloud in R^3'];
for i=2:m+1
   X=rand(3,n);
   ToyData{i,1}=X;  
end
...
for i=2:m+1
    Centers=rand(3);  
    a=max(randi(floor(.45*n)),floor(.1*n)); %random number of points for first cluster, can be at most 45% of all points, but is at least 10%
    b=max(randi(floor(.45*(n-a))), floor( (n-a)*.1)); %random number of pts in 2nd cluster, at least 10% remaining, at most 45%
    c=n-a-b; 
    ...
    %random number of points for each small cluster, can be at most 45% and
    %at least 10%
    a1=max(randi(floor(.45*a)),floor(.1*a)); 
    a2=max(randi(floor(.45*(a-a1))),floor((a-a1)*.1)); 
    a3=a-a1-a2;    
    
    X=cat(2,repmat(Centers(:,1),1,a1),repmat(Centers(:,2),1,a2),repmat(Centers(:,3),1,a3));
       
    b1=max(randi(floor(.45*b)),floor(b*.1)); %random number of points for first small cluster, can be at most 45% of all points and
    %at least 10%
    b2=max(randi(floor(.45*(b-b1))),floor((b-b1)*.1)); 
    b3=b-b1-b2;  
    
    Y=cat(2,repmat(Centers(:,4),1,b1),repmat(Centers(:,5),1,b2),repmat(Centers(:,6),1,b3));
    
    c1=max(randi(floor(.45*c)),floor(c*.1)); %random number of points for first small cluster, can be at most 45% of all points and
    %at least 10%
    c2=max(randi(floor(.45*(c-c1))), floor((c-c1)*.1)); 
    c3=c-c1-c2;  
...
ToyData{1,7}=['Random Cloud in R^2'];
for i=2:m+1
   X=rand(2,n) * sqrt(pi);
   X=[X; ones(1, size(X, 2)) * 0.5];
   ToyData{i,7}=X;
end
