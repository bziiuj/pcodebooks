function [ PIs ] = new_make_PIs(interval_data, res, sig, weight_func, ...
	persistence_limits, type, max_b_p)
%, parallel)
...
%            options specified for the provided interval data.
	...
	if type==1       
		[ data_images ] = hard_bound_PIs( b_p_data, max_b_p_Hk, weight_func, persistence_limits, res, sig);
	elseif type==2
		[ data_images ] = soft_bound_PIs( b_p_data, max_b_p_Hk, weight_func, persistence_limits, res, sig);
	else
...
%%% for future improvement, different sigma for hight and width
...
%         
	...
	for k=1:o
		...
		%        end
				D=find(C<0);
				F=find(B(:,1)<0);
				if length(D)>0 || length(F)>0
					problems=[problems; j,i,k];
				elseif length(D)==0 && length(F)==0
					problems=problems;
				end
			end
		end
